...
"""
Custom transformers
you will want to convert an existing Python function into a transformer 
to assist in data cleaning or processing. You can implement a transformer 
from an arbitrary function with FunctionTransformer. For example, 
...
def daqfft():
    try:
        ...
        custom_func = int(request.form['chose_func'])
...
# Operation: Generating polynomial features
        ...
        if operation == 'Transformer':
            transformer = FunctionTransformer(custom_func)#eg.np.log1p
            custom_tran = transformer.transform(X_train)            
            result= jsonify(custom_tran)
        return result
